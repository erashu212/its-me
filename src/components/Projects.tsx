              ...
              {projects.map((project, index) => (
                ...
                >
                  <div className="text-center">
                    ...
                    <div className="text-xs opacity-75">{project.company}</div>
                  </div>
                </motion.button>
              ))}
            </div>
                ...
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className={`px-3 py-1 rounded-full text-xs border ${getStatusColor(projects[activeProject].status)}`}>
                ...
                <div className="mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {projects[activeProject].technologies.map((tech, index) => (
                      <span
                        key={index}
                        className="px-3 py-1 bg-primary-50 text-primary-700 text-xs rounded-full border border-primary-200 hover:bg-primary-100 transition-colors duration-200"
                      >
                        {tech}
                      </span>
                    ))}
                  ...
                  <div className="flex gap-4">
                    {projects[activeProject].links.map((link, index) => (
                      <motion.a
                        ...
                        whileTap={{ scale: 0.95 }}
                        className="flex items-center gap-2 px-4 py-2 bg-primary-50 text-primary-700 rounded-lg hover:bg-primary-100 transition-all duration-200 border border-primary-200"
                      >
                ...
                <div className="grid grid-cols-2 gap-4">
                  {Object.entries(projects[activeProject].impact).map(([key, value], index) => (
                    <motion.div 
                      ...
                      transition={{ delay: index * 0.1 }}
                      className="text-center p-4 bg-gradient-to-br from-primary-50 to-primary-100 rounded-lg border border-primary-200 hover:shadow-soft transition-all duration-200"
                    >
                      <div className="text-2xl font-bold text-primary-700 mb-1">{value}</div>
